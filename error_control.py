...
#         print('\n')
    ...
    if error_dec == 0 :
        error_message = '%s%sError bit at position: None%s' %(fg("green"),attr("bold"),attr("reset"))
    else:
        color_bit = list(codeword)
        color_bit.reverse()
        before = ''.join(color_bit[:error_dec*-1])
        after = ''.join(color_bit[(error_dec*-1):])
        error_color = '%s%s%s%s%s%s%s' %(fg('green'),before,fg('red'),after[0], fg("green"),after[1:],attr("reset"))
        error_message = '%s%sFound an error:        %s %s [position:%d]' %(fg('green'),attr("bold"),error_color,attr("bold"), error_dec)
        color_bit = list(codeword)
        color_bit.reverse()
        before = ''.join(color_bit[:error_dec*-1])
        after = ''.join(color_bit[(error_dec*-1):])
        curr = '1' if after[0] == '0' else '1'
        error_color = '%s%s%s%s%s%s%s' %(fg('green'),before,fg('red'),curr, fg("green"),after[1:],attr("reset"))
        error_message += '\n%sLet me fix it:         %s%s%s' %(fg("green"),attr("bold"),error_color,attr("reset"))
    pfor = "%s[Output] : %s %s %s" %(fg("green"),fg("blue"),error_message,attr("reset"))
...
def main():
    ...
    type_func = {
        ...
        2 : ("%s%sParity bit" %(fg("green"),attr("bold"))) + (" %s(Checker)"%(fg("blue"))),
        3 : ("%s%sChecksum" %(fg("green"),attr("bold"))) + (" %s(Generator)"%(fg("red"))),
        4 : ("%s%sChecksum" %(fg("green"),attr("bold"))) + (" %s(Checker)"%(fg("blue"))),
        5 : ("%s%sCRC" %(fg("green"),attr("bold"))) + (" %s(Generator)"%(fg("red"))),
        6 : ("%s%sCRC" %(fg("green"),attr("bold"))) + (" %s(Checker)"%(fg("blue"))),
        7 : ("%s%sHamming code" %(fg("green"),attr("bold"))) + (" %s(Generator)"%(fg("red"))),
        8 : ("%s%sHamming code" %(fg("green"),attr("bold"))) + (" %s(Checker)"%(fg("blue"))),
    ...
    print("%s%s%s%s" %(fg("blue"),attr("bold"),welcome_message,attr("reset")))
    print(colored("----------------------------------------------------------------","green"))
    time.sleep(1)
    ...
    print("%s%s        ctrl+c to exit       %s"%(fg("white"),bg("red"),attr("reset")))
    select_fun = int(input(colored("Select function: ","blue")))
    if select_fun not in type_func:
        ...
        if select_fun in [1,2]:
            ...
            print("%s[2] : 1D %s(Odd)  " %(fg("green"),fg("blue")));
            print("%s[3] : 2D %s(Even) " %(fg("green"),fg("red")));
            print("%s[4] : 2D %s(Odd)  " %(fg("green"),fg("blue")));
            select_option = int(input(colored("Select type: ", "blue")))
            ...
            if select_option not in [1,2,3,4]:
                ...
                if select_fun == 1: size_dataword = int(input(colored("Input size of dataword(>= 5): ","blue")))
                dataword = str(input(colored("Input frame seperate by space( ): ","blue")))
                try :
                    dataword = dataword.split(' ')
                    if checkstring(dataword) == True:
                        return print("%s%sInvalid input!%s" %(fg("red"),attr("bold"),attr("reset")))
                    
                    print("%s%s[Input]  : %s %s" %(fg("green"),attr("bold"),fg("blue"),' '.join(dataword)))
                    if select_fun == 1:
                        codeword = parity_gen(dataword,size_dataword,parity_option[select_option],len(dataword))
                        codeword = colored(codeword, "blue")
                    else :
                        codeword = parity_check(dataword,parity_option[select_option],len(dataword))
                        codeword = colored('No damage','blue') if codeword == 1 else colored('Damage!', 'red')
                    print("%s%s[Output] : %s %s %s" %(fg("green"),attr("bold"),fg("blue"),codeword,attr("reset")))

                except :
                    return print("%s%sError !%s" %(fg("red"),attr("bold"),attr("reset")))
        elif select_fun in [3,4]:
            size_dataword = int(input(colored("Input size of dataword(>= 5): ","blue")))
            dataword = str(input(colored("Input frame seperate by space( ): ","blue")))
            try :
                dataword = dataword.split(' ')
                if checkstring(dataword) == True:
            ...
            size_dataword = int(input(colored("Input size of dataword(>= 5): ","blue")))
            dataword = str(input(colored("Input data: ","blue")))
            try :
                if checkstring(dataword) == True:
                    return print("%s%sInvalid input!%s" %(fg("red"),attr("bold"),attr("reset")))
                ...
                select_crc = int(input("%sSelect divisor: %s" %(fg("blue"),attr("reset"))))
                if select_crc not in [0,1,2,3,4,5,6,7] : print("%s%sInvalid input!%s" %(fg("red"),attr("bold"),attr("reset")))
                if select_crc == 7:
                    crc = str(input(colored("Input divisor: ","blue")))
                else:
                    crc_values = list(crc_dict.values())
                    crc = crc_values[select_crc-1]
                print("%s%s[Input]   : %s %s" %(fg("green"),attr("bold"),fg("blue"),dataword))
                print("%s[Divisor] : %s %s" %(fg("green"),fg("blue"),crc))
                if select_fun == 5:
                    codeword = CRC_gen(dataword,size_dataword,crc)
                    codeword = colored(codeword+f' size[{len(codeword)}]', "blue")
                else :
                    codeword = CRC_check(dataword,size_dataword,crc)
                    codeword = colored('No damage','blue') if codeword == 1 else colored('Damage!', 'red')
                print("%s[Output]  : %s %s %s" %(fg("green"),fg("blue"),codeword,attr("reset")))
            except :
                return print("%s%sError !%s" %(fg("red"),attr("bold"),attr("reset")))
            ...
            try :
                ...
                return print("%s%sError !%s" %(fg("red"),attr("bold"),attr("reset")))
        elif select_fun in [9]:
            dataword = str(input(colored("Input data: ","blue")))
            try :
                if checkstring(dataword) == True:
                    return print("%s%sInvalid input!%s" %(fg("red"),attr("bold"),attr("reset")))
                prob = float(input(colored("Input probability(0.0-1.0):","blue")))
                unreliable_transmission(dataword,prob)
            except :
                return print("%s%sError !%s" %(fg("red"),attr("bold"),attr("reset")))
